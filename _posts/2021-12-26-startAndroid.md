...
XML 파일에서 각 View를 구분하기 위해 id를 지정하는데 이때도 `R.java`가 이용된다. id 속성을 추가하면 자동으로 `R.java` 파일에 상수 변수로 id가 추가된다. `android:id = "@+id/유니크한 이름`으로 id를 지정하게 되는데 이때 `@`의 의미가 `R.java`를 의미한다. 즉, +가 추가를 나타낸다고 추측해보면 `R.java`에 id를 추가하라는 말이된다. 


## 매니페스트 파일
---
안드로이드 앱의 메인 환경 파일이다. 시스템은 매니페스트 파일을 바탕으로 앱을 실행하므로 각 컴포넌트들을 매니페스트 파일에 등록해야 시스템이 인지할 수 있다. 
런타임 때 매니페스트 파일의 정보를 참조하여 앱을 실행하는데, 개발한 컴포넌트를 등록해놓지 않으면 해당 컴포넌트는 앱이 찾을 수 없어 실행할 수 없게된다. 

매니페스트 파일에 들어가는 태그를 간략하게 소개하면 다음과 같다.
...
<img width="190%" hegith="100%" alt="android os" src="https://user-images.githubusercontent.com/46019755/147407123-3b1ced31-8dbe-44f3-979b-9fbc5fb85db4.png">
